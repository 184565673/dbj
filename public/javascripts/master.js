...
$('#createBox .btn-primary').on('click', function() {
	...
	var oldVersion =  _.max(vers) ;

	if(String(oldVersion) > String(version)) {
		alert('新版本号必须大于现有版本号');
		return false;
	}else if(_.contains(vers, version)) {
		alert('版本已存在');
		return false;
	}
	var $btn = $(this).button('loading');
	$.getEx(svrUrl,'/master/create', {version: version, oldVersion : oldVersion}, function(rtnData) {
		$btn.button('reset')
		if( rtnData.rtn ==0) {
...
$('#delBox .btn-danger').on('click', function() {
	...
	$.getEx(svrUrl,'/master/del', {version: version}, function(rtnData) {
		$btn.button('reset');
		if( rtnData.rtn == 0) {
...
$('#deployBox .btn-info').on('click', function() {
	...
	$.getEx(svrUrl,'/master/deploy', {version: version}, function(rtnData) {
		$btn.button('reset');
		if( rtnData.rtn == 0) {
